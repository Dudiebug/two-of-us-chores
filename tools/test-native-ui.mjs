...
for (const initial of ['prompt','granted','denied']) {
  ...
  const plugin={getStatus:async()=>({notifications:permission,configured}),requestPermissions:async()=>{asked++;permission='granted';return {notifications:permission}},configure:async()=>{configured=true},disable:async()=>{configured=false},openSettings:async()=>{},pollNow:async()=>({posted:++posted})};
  const context={window:{Capacitor:{getPlatform:()=> 'android',Plugins:{ChoresNotifications:plugin}}},navigator:{userAgent:'ChoresCapacitor/1.2'},document:{querySelector:(s)=>{if(!nodes.has(s))nodes.set(s,{});return nodes.get(s)},addEventListener:()=>{}},localStorage:{getItem:(k)=>storage.get(k),setItem:(k,v)=>storage.set(k,v),removeItem:k=>storage.delete(k)},crypto:{randomUUID:()=> 'test-native-device'},fetch:async()=>({ok:true,text:async()=>JSON.stringify({token:'test-token-long-enough',cursor:0})}),setTimeout,Date,console};
