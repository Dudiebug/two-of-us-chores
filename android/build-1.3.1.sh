...
s=re.sub(r'versionName\s+"[^"]+"', 'versionName "1.3.1"',s,count=1)
s=s.replace('dependencies {','''dependencies {
...
grep -q 'versionName "1.3.1"' android/app/build.gradle
...
cd ..
mkdir -p dist
cp android/app/build/outputs/apk/release/app-release-unsigned.apk dist/chores-1.3.1-unsigned.apk
cp android/app/build/outputs/bundle/release/app-release.aab dist/chores-1.3.1-unsigned.aab
